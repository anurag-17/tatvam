...
export const Home = () => {
    ...
    useEffect(() => {
        async function loadPosts() {
            const response = await fetch('https://controlf5.co.in/client-demo/react-wordpress/wp-json/wp/v2/media/28');
            const posts = await response.json();
            setPosts(posts.guid.rendered);
        }
        async function homepagecontent() {
            const homepagedata = await fetch('https://controlf5.co.in/client-demo/react-wordpress/wp-json/wp/v2/pages/33');
